...
You are producing one candidate design in architect's parallel exploration. Read [`SKILL.md`](SKILL.md) in full first. That's the workflow you're inside. Output a candidate design package: type sketch, function signatures, module map, and prose rationale shaped per [`rationale-template.md`](RATIONALE-TEMPLATE.md).
...
- Shared state: if two actors might both write, ask "what happens?" If the answer isn't "nothing," default to per-actor state with a merge at the read boundary, per the separate before serializing shared state principle ([REFERENCE.md](../principles/REFERENCE.md)).
...
- Encode invariants in types: hard-to-misuse types > runtime checks > prose comments, per the encode lessons in structure principle ([REFERENCE.md](../principles/REFERENCE.md)).
- Validate at boundaries, trust types inside, per the boundary discipline principle ([REFERENCE.md](../principles/REFERENCE.md)). Business logic as pure functions. The shell stays thin.
- Single source of truth per invariant. Derive instead of sync.
- Idempotent state transitions where applicable, per the make operations idempotent principle ([REFERENCE.md](../principles/REFERENCE.md)). Ask what happens if the operation runs twice or crashes halfway.
- Short call chains. If tracing the flow needs more than three files, flatten the hierarchy, per the laziness protocol and minimize reader load principles ([REFERENCE.md](../principles/REFERENCE.md)).
