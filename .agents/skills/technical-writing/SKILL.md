...
description: Layered doc standard (Diátaxis mode, Google developer style, STE, Global English). Use when writing or reviewing README, ROADMAP, COMPARISON, CHANGELOG or changeset text, JSDoc that ships in dist, PR descriptions, or commit messages.
---
...
- Indent code snippets with 2 spaces, the repo's indent width. Write real paths and real symbols. Make every count or tree claim true at the commit that lands it, and include the command that regenerates it.
